...
defineFeature(feature, test => {
    let AppWrapper;
    test('When user hasn’t specified a number, 25 is the default number', ({ given, when, then }) => {
        ...
        then('the user sees 25 events by default', () => {
            AppWrapper.update();
            expect(AppWrapper.state('numberOfEvents')).toEqual(25);
        });
    });
